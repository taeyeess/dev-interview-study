...
개발자는 직관적으로 코드에서 찝찝함을 느낀다. 이를 나쁜 냄새가 나는 코드라고 *클린 코드*에서 이야기한다. 직관이라는 것은 말로 설명하기 어려운 주관적 영역에 속한다. 지금까지 활동해 온 선배 개발자들은 이러한 직관들에 이름을 붙이고 이에 대해 약속(Convetion)을 구축하고 우선순위를 정하며 하나의 합의를 도출해냈다. 대표적인 예시로, _Camel Case_, _pascal case_, *snake case*등과 같은 `이름 규약(name convention)`이 발생했고 지금 우리는 이러한 규범에 따라 코드를 작성하고 있다.
...
> We don't read code, we decode it. - _Peter Seibel_
...
디자인(설계) 단계에서 프로그래밍 목적을 반드시 **미리 정의** 를 해야만 하고,
